...
const metrics = [
  ...
  { label: "Style", value: "Design x Engineering" },
  { label: "Output", value: "Static-first Portfolio" }
];

export function Hero({ name, role, intro, summary }: HeroProps) {
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-[var(--border)] bg-[var(--panel)] px-6 py-12 shadow-[var(--shadow-soft)] sm:px-8 sm:py-16 lg:px-12">
      ...
      <div className="relative grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
        <div className="space-y-8">
          <div className="inline-flex items-center gap-3 rounded-full border border-[var(--border)] bg-[var(--panel-strong)] px-4 py-2 text-sm text-[var(--muted)]">
          ...
          <div className="space-y-6">
            ...
            <p className="max-w-2xl text-2xl font-medium tracking-[-0.03em] text-[var(--foreground-soft)] sm:text-3xl">
              {role}
            </p>
        ...
        <div className="grid gap-4">
          {metrics.map((metric) => (
            <div
              key={metric.label}
              className="rounded-[1.5rem] border border-[var(--border)] bg-[linear-gradient(180deg,var(--panel-strong),transparent)] p-5"
            >
          ...
          <div className="rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface-code)] p-5 font-mono text-sm text-[var(--foreground-soft)]">
            ...
            <p className="mt-3">{'const value = "build clean products with strong UX and solid engineering";'}</p>
          </div>
        </div>
      </div>
    </section>
