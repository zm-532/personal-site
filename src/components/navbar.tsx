    ...
    <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-[color:var(--bg-header)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          <nav className="hidden items-center gap-1 rounded-full border border-[var(--border)] bg-[var(--panel)] p-1 md:flex">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-full px-4 py-2 text-sm text-[var(--muted)] transition duration-300 hover:bg-[var(--panel-strong)] hover:text-[var(--foreground)]"
              >
                {item.label}
              </Link>
            ))}
